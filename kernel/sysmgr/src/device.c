 ...
 */
EXPORT DevCB* searchDevCB( UB *devnm )
{
	QUEUE	*q;
	DevCB	*devcb;
 ...
 * Device registration
 */
LOCAL ID _tk_def_dev( UB *devnm, T_DDEV *ddev, T_IDEV *idev, VP caller_gp )
{
	DevCB	*devcb;
	INT	len, evttyp;
	ER	ercd;
 ...
 *	device name (pdevnm).
 */
EXPORT INT phydevnm( UB *pdevnm, UB *ldevnm )
{
	UB	c;
	INT	unitno;
 ...
 */
LOCAL ID _tk_ref_dev( UB *devnm, T_RDEV *rdev )
{
	UB	pdevnm[L_DEVNM + 1];
	DevCB	*devcb;
	INT	unitno;
 ...
 */
LOCAL INT _tk_evt_dev( ID devid, INT evttyp, VP evtinf )
{
	DevCB	*devcb;
	EVTFN	eventfn;
	VP	exinf;
 ...
 * Extension SVC entry
 */
LOCAL INT devmgr_svcentry( VP pk_para, FN fncd, VP caller_gp )
{
	ER	ercd;
