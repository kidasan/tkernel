 ...
 */
#define ROUND_RESBLKSZ(resblksz)	(INT)((UINT)((resblksz) + 3) & ~0x00000003U)
 ...
 */
SYSCALL ER _tk_def_ssy P2( ID ssid, T_DSSY *pk_dssy )
{
	SSYCB	*ssycb;
	VP	resblk;
	SSYCB	**prev;
...
SYSCALL ER _tk_sta_ssy( ID ssid, ID resid, INT info )
{
	...
	save_texflg = tcb->texflg;
	tcb->texflg |= SSFN_RUNNING;
	tcb->sysmode++;
	END_CRITICAL_NO_DISPATCH;
	...
	BEGIN_CRITICAL_SECTION;
	tcb->sysmode--;
	tcb->texflg = save_texflg;
	END_CRITICAL_NO_DISPATCH;

	/* Processing if an exception occurs while suspending a
	   task exception */
	if ( tcb->exectex != 0 ) {
		call_brkhdr(tcb);	/* Execute break function */
	}
 ...
 */
EXPORT ER svc_ientry P2GP( VP pk_para, FN fncd )
{
	ID	ssid;
	SSYCB	*ssycb;
	ID	save_execssid;
	...
	if ( in_indp() ) {
		...
		if ( (tcb->waitmask & TTX_SVC) != 0 ) {
			UnlockSVC();
			return E_DISWAI; /* Disable extended SVC call */
		}

		BEGIN_CRITICAL_SECTION;
		save_execssid = tcb->execssid;
		save_waitmask = tcb->waitmask;
		save_exectex  = tcb->exectex;
		tcb->execssid = ssid;
		tcb->waitmask = 0;
		...
		BEGIN_CRITICAL_SECTION;
		tcb->sysmode--;
		tcb->execssid = save_execssid;
		tcb->waitmask = save_waitmask;
		tcb->exectex |= save_exectex;
		END_CRITICAL_NO_DISPATCH;
 ...
 */
SYSCALL ER _tk_def_tex( ID tskid, T_DTEX *pk_dtex )
{
	TCB	*tcb;
	ER	ercd = E_OK;
...
EXPORT void call_brkhdr( TCB *tcb )
{
	...
	if ( tcb->priority < ctxtsk[prid]->priority ) {
		/* Raise its own task priority to the same level of the task 
		   running extended SVC */
		change_task_priority(ctxtsk[prid], tcb->priority);
	}

	/* Suspend the task exception during break function is running */
	save_texflg = ctxtsk[prid]->texflg;
	ctxtsk[prid]->texflg |= SSFN_RUNNING;
	ctxtsk[prid]->sysmode++;
	...
	prid = get_prid();
	ctxtsk[prid]->sysmode--;
	ctxtsk[prid]->texflg = save_texflg;

	/* Set the task priority back to the original level */
#ifdef NUM_MTXID
	priority = chg_pri_mutex(ctxtsk[prid], ctxtsk[prid]->bpriority);
#else
	priority = ctxtsk[prid]->bpriority;
#endif
	if ( ctxtsk[prid]->priority != priority ) {
		change_task_priority(ctxtsk[prid], priority);
	}
	END_CRITICAL_SECTION;
}
...
SYSCALL INT _tk_end_tex( BOOL enatex )
{
	...
	if ( (tcb->texflg & (TEX0_RUNNING|TEX1_RUNNING)) != TEX1_RUNNING ) {
		return E_CTX;
	}

	BEGIN_CRITICAL_SECTION;
	texptn = tcb->pendtex & ~0x00000001U;  /* Except exception code 0 */
	if ( texptn != 0 ) {
		while ( (texptn & 1) == 0 ) {
			texcd++;
			texptn >>= 1;
 ...
 */
SYSCALL ER _tk_get_res( ID resid, ID ssid, VP *p_resblk )
{
	INT	idx;
	SSYCB	*ssycb;
	ER	ercd = E_OK;
