...
/*
 *	smpmemmgr.c (T-Kernel/SM)
 *	No MMU version: smp memory management
 */
...
#define OBJNAME_MMLOCK	(UB*)"Mem"		/* Multi-lock for Memory Manager */
...
{
	adr = CachingAddr(adr);
	if ( adr >= (VP)pt->top_page && adr < (VP)(pt->top_page + pt->maxpage) ) {
